...
class PostViewsTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="test-user", password="123456789q"
        )
    ...
    def test_post_detail_404(self):
        response = self.client.get(
            reverse("blog:post_detail", args=["Page-doesnt-exist"])
        )
...
class CreateViewsTest(TestCase):
    def setUp(self):
        ...
        self.category_first = Category.objects.create(
            title="test-category-first", slug="test-category-first"
        )
        self.category_second = Category.objects.create(
            title="test-category-second", slug="test-category-second"
        )
        self.post_first = Post.objects.create(
            title="test-title-first",
            slug="test-slug-first",
            content="test-content",
            category=self.category_first,
            author=self.user,
            published=True,
        )
